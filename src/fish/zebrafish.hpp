    ...
    public:
        ...
        void move() override
        {
            ...
            if (!this->is_robot()) {
                ...
                this->heading() = Params::fish_in_ring::heading_robot;

            // a fish will move to the next position with some uncertainty,
            // i.e. this means the fish might not move at all
            bool move;
            (this->is_robot()) ? (move = tools::random_in_range(0.0f, 1.0f) < Params::fish_in_ring::prob_move)
        ...
        template <typename Shoal> void calc_intuitions(const Shoal& shoal, size_t num_cells_look) const
        {
