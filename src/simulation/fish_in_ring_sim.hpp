...
namespace sim {
template <typename Params, typename FishType,
          typename Stat = stat::StatBase<Params>>
class FishInRing : public SimulatorBase<Params, Stat> {
 public:
  FishInRing()
      : num_agents_(Params::fish_in_ring::num_fish +
                    Params::fish_in_ring::num_robot),
        cell_degree_(360.0 / Params::fish_in_ring::num_cells),
        num_cells_look_(
            std::ceil(Params::fish_in_ring::deg_vision / cell_degree_)) {
    fish_.resize(num_agents_);
    int robots_to_assign = Params::fish_in_ring::num_robot;
    for (auto& f : fish_) {
      f.position() = std::floor(tools::random_in_range(0.0f, 1.0f) *
                                Params::fish_in_ring::num_cells);
      if (robots_to_assign-- > 0) f.is_robot(true);
    }
  }

  void spin_once() override {
    using namespace types;
    std::vector<Heading> new_headings;
    for (size_t i = 0; i < num_agents_; ++i) {
      ...
      boost::push_back(neighborhood,
                       boost::irange(fish_[i].position(),
                                     fish_[i].position() +
                                         fish_[i].heading() *
                                             static_cast<int>(num_cells_look_) +
                                         fish_[i].heading(),
                                     fish_[i].heading()));
      ...
      for (size_t j = 0; j <= num_cells_look_; ++j) {
        ...
        if (neighbors.size() > 0) {
          ...
          std::shuffle(neighbors.begin(), neighbors.end(),
                       tools::random_device_);
          for (size_t k = 0; k < neighbors.size(); ++k) {
            ++num_neighbors;
            sum_heading += fish_[neighbors.at(k)].heading();
            if (num_neighbors == Params::fish_in_ring::max_neighbors) break;
          }
        }
        if (num_neighbors == Params::fish_in_ring::max_neighbors) break;
      }  // for j (num_cells_look_)

      // Save the new headings - No probabilistic decisions at this point
    ...
    for (size_t i = 0; i < fish_.size(); ++i)
      fish_[i].heading() = new_headings[i];

    std::for_each(fish_.begin(), fish_.end(), [](FishType& f) {
      if (!f.is_robot()) {
        bool obey = tools::random_in_range(0.0f, 1.0f) <
                    Params::fish_in_ring::prob_obedience;
        if (!obey) f.heading() = reverse_heading(f.heading());
      } else
        f.heading() = Params::fish_in_ring::heading_robot;
      // for now robotic fish will explicitly stick to their heading

      bool move;
      (f.is_robot()) ? (move = tools::random_in_range(0.0f, 1.0f) <
                               Params::fish_in_ring::prob_move)
                     : (move = tools::random_in_range(0.0f, 1.0f) <
                               (1 - Params::fish_in_ring::prob_stay));
      if (move)
        f.position() = (f.position() + f.heading()) %
                       static_cast<int>(Params::fish_in_ring::num_cells);
    });
  }
 ...
 private:
  std::vector<size_t> find_neighbors(int neighborhood, size_t self) const {
    ...
    for (size_t i = 0; i < fish_.size(); ++i) {
      if (i == self) continue;
      if (fish_[i].position() == neighborhood) neighbors.push_back(i);
    }
    return neighbors;
  }

  int neighbor_heading_sum(const std::vector<size_t>& idcs) const {
    int sum_heading = 0;
    for (size_t i = 0; i < idcs.size(); ++i)
      sum_heading += fish_[idcs[i]].heading();
    return sum_heading;
  }
